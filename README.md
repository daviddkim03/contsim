...
- 3D view with orbit and zoom, a container switcher and hover highlighting per box type. It stays smooth with a thousand boxes in a container: each box type is drawn in a couple of calls rather than two per box.
...
- **Fragile** boxes, marked per box type, travel upright and never carry another box on top. They are given their places first and the rest of the load is packed beneath them, so they ride on top of it, each resting on at least half its base. Since nothing goes on top of them, an order that is mostly fragile needs more containers than its volume suggests.
