...
- `src/ui/viewer3d.ts` renders on demand, not in a loop. Core x = length, y = width, z = height; the scene maps height to y and width to z (`viewerMath.ts`). The 3D view is the only view of the packing (the placement table is gone) and the container outline is always drawn; the toolbar is the container switcher and Reset view, nothing else.
- Drawing scales by box type, not by box: one InstancedMesh over a shared unit cube plus one merged edge buffer per type. Keep it that way; a mesh per box cost 25 fps at 900 boxes (PROJECT.md 5.8).
- A view change (hover, container) must stay near-free at any box count: the legend skips identical states, and hovering only touches materials.
- Fragility is per box type (`BoxType.fragile`), and it carries two rules at once: upright only and nothing above it. Fragile boxes go first: each is reserved a place with its top at the roof over the first free candidate point, the load packs beneath (never so that the fragile box would rest on less than half its base; a box too small to carry half alone goes in together with enough copies of itself beside it), and at the end each comes down onto what is under it. Never pack them last instead: the load takes every column to the roof and they spill into extra containers. There is no wall rule (it emptied the middle of the container) and no global "keep upright" any more (PROJECT.md 4.5).
- Even mode learns the container count from a plain fill, then also tries the spread over fewer containers while everything fits; a balanced mix packs better than a greedy fill's leftovers.
...
- Excel export: `src/ui/report.ts` turns the state into one sheet per container and `src/ui/plan.ts` builds the two 2D views as grids of coloured cells, `src/ui/xlsx.ts` writes SpreadsheetML, `src/ui/zip.ts` writes the archive. All dependency-free; `tests/helpers/unzip.ts` reads the result back independently.
- Import: `src/ui/spreadsheet.ts` reads .xlsx (all sheets, shared strings) and .csv; `src/ui/orderImport.ts` maps rows to box rows and recognizes the app's own export (the Container sheets restore the whole scenario: the settings from the first, the boxes added up across all of them). The file is parsed twice: once to fill the dialog (`src/ui/importDialog.ts`, which asks for the unit unless the file states one), then again in the unit chosen. There is no JSON import or export any more; `parseDraft` / `serializeDraft` only serve localStorage.
