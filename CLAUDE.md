...
- A view change (hover, layer, container) must stay near-free at any box count: the legend skips identical states, and the viewer only touches draw ranges and materials.
- Fragility is per box type (`BoxType.fragile`), and it carries three rules at once: upright only, touching one of the container's four vertical walls, and nothing above it. Fragile boxes go first: each is reserved a wall place with its top at the roof, the load packs beneath (never so that the fragile box would rest on less than half its base), and at the end each comes down onto what is under it. Never pack them last instead: the load takes every wall column to the roof and they spill into extra containers (PROJECT.md 4.5). There is no global "keep upright" any more.
